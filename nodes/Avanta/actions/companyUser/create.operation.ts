...
export async function execute(
	...
	for (let i = 0; i < items.length; i++) {
		try {
			...
			if (!bulk) {
				let responseData = await magentoApiRequest.call(this, 'POST', '/V1/proline-admin/company/saveUserAndCompanyUser', companyUser);
	...
	if (bulk) {
		try {
			let responseData = await magentoApiRequest.call(this, 'POST', '/async/bulk/V1/proline-admin/company/saveUserAndCompanyUser', data);
			const executionData = this.helpers.constructExecutionMetaData(
				this.helpers.returnJsonArray(responseData as IDataObject[]),
				{itemData: {item: 0}},
			);
